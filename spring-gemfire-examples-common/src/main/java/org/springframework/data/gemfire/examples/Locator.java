...
public class Locator {
    ...
    public static void main(String args[]) {
        ...
        AdminDistributedSystem adminDistributedSystem = AdminDistributedSystemFactory.getDistributedSystem(distributedSystemConfig);
            ...
            try {
                if (options.get("command").equals("stop")) {
                    if (stopLocator(locator, MAX_WAIT_TIME)) {
                        System.out.println(String.format("locator stopped on %s[%s]", locator.getConfig().getHost(), locator.getConfig().getPort()));
                    } else {
                        System.out.println(String.format("failed to stop locator on %s[%s]", locator.getConfig().getHost(), locator.getConfig().getPort()));
                    }
                }
                ...
                if (options.get("command").equals("start")) {
                    ...
                    if (startLocator(locator, MAX_WAIT_TIME)) {
                        System.out.println(String.format("locator running on %s[%s]", locator.getConfig().getHost(), locator.getConfig().getPort()));
                        for (Entry<Object, Object> prop : locator.getConfig().getDistributedSystemProperties().entrySet()) {
                            System.out.println(prop.getKey() + "=" + prop.getValue());
                        }
                    } else {
                        System.out.println(String.format("failed to start locator on %s[%s]", locator.getConfig().getHost(), locator.getConfig().getPort()));
                    }
                }
    ...
    static boolean stopLocator(DistributionLocator locator, int maxWaitTime) throws AdminException, InterruptedException {
    ...
    static boolean startLocator(DistributionLocator locator, int maxWaitTime) throws AdminException, InterruptedException {
        if (!locator.isRunning()) {
            locator.start();
            int waitTime = 0;
            while (waitTime < maxWaitTime && !locator.isRunning()) {
