...
public class SpringJavaBasedContainerGemFireConfigurationTest {
    ...
    @Resource(name = "ExamplePartition") private Region<Object, Object> examplePartition;
    ...
    protected void assertExpirationAttributes(ExpirationAttributes expirationAttributes, ExpirationAction expectedAction, int expectedTimeout) {
    ...
    protected void assertPartitionAttributes(PartitionAttributes partitionAttributes, int expectedLocalMaxMemory, int expectedRedundantCopies, int expectedTotalMaxMemory) {
    ...
    protected void assertRegion(Region<?, ?> region, String expectedNamePath, DataPolicy expectedDataPolicy) {
        assertRegion(region, expectedNamePath, String.format("%1$s%2$s", Region.SEPARATOR, expectedNamePath), expectedDataPolicy);
    }
    ...
    public void testPartitionRegionConfiguration() {
        ...
        assertExpirationAttributes(examplePartition.getAttributes().getEntryIdleTimeout(), ExpirationAction.INVALIDATE, 300);
        assertExpirationAttributes(examplePartition.getAttributes().getEntryTimeToLive(), ExpirationAction.DESTROY, 600);
        assertEvictionAttributes(examplePartition.getAttributes().getEvictionAttributes(), EvictionAction.LOCAL_DESTROY, EvictionAlgorithm.LRU_MEMORY, examplePartition.getAttributes().getPartitionAttributes().getLocalMaxMemory());
    }
}
